...
vel_res = plot_velocity_arrows(tsne_res = tsne_res, 
                               cell_a = "MCF10A", cell_b = "MCF7", 
                               p = img_res$plot, max_plotted = 500)
vel_res[[2]] = vel_res[[2]] + guides(fill = "none")
plot_grid(plotlist = vel_res, rel_widths = c(2, 1))
```

```{r anim}
dt_anim = plot_velocity_arrows(tsne_res = tsne_res, return_data = TRUE,
                               cell_a = "MCF10A", cell_b = "MCF7", 
                               p = img_res$plot, max_plotted = Inf)
dt_anim[, angle_bin := round(angle / 360)]
p_anim = ggplot(dt_anim, 
    ...
    scale_color_gradientn(colours = c("orange", "red", 
                                      "purple", "blue",
                                      "green", "orange"), 
                          limits = c(0, 360), breaks = 0:4*90)
p_anim + transition_states(angle_bin) 
